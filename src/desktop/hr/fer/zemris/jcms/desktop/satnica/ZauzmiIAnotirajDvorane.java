...
public class ZauzmiIAnotirajDvorane {
	...
	public static void main(String[] args) throws IOException {
		...
		List<ScheduleEntry> schedule = readSchedule(fileName, onlyRooms, semester);
		List<ScheduleEntry> allocSchedule = new ArrayList<ScheduleEntry>(schedule.size());

		String datesFrom = "2010-06-14";
		String datesTo = "2010-06-18";

		nadodajFiksnaZauzeca(allocSchedule, "D:\\fer\\ferko\\0910L\\effective\\in\\vanjskaDvorane", datesFrom, datesTo, onlyRooms, semester);
		...
		for(ScheduleEntry entry : allocSchedule) {
			...
			switch(iStatus) {
			case 0: // zauzmi
				String context = entry.courseInstanceID+"#"+entry.kind+"#"+entry.value+"#"+entry.description;
...
*/
	...
	private static void nadodajFiksnaZauzeca(List<ScheduleEntry> allocSchedule, String dirName, String fromDate, String toDate, Set<String> allRooms, String semester) throws IOException {
		for(File f : new File(dirName).listFiles()) {
			...
			while(true) {
				...
				String sTo = elems[3];
				String sKind = elems[4];
				String sValue = elems[5];
				String sDescr = elems[6];
				String isvu = elems[7];
				String name = elems[8];
				...
				se.lab = "LAB";
				se.room = room;
				se.courseInstanceID = semester+"/"+se.isvuCode;
				se.kind = sKind;
				se.value = sValue;
				se.description = sDescr;
	...
	public static Boolean allocateRoom(String url, Integer login_id, String room, String userCode, String dateTimeFrom, String dateTimeTo, String reason, String context) {
		if(!alokacija) {
		...
		client.setTransportFactory(new XmlRpcCommonsTransportFactory(client));
		client.setConfig(config);
		Object[] cookies = context==null ? new Object[] {} : new Object[] {"context="+context};
		Object[] params = new Object[] {login_id,room,userCode, from, to, reason, cookies};
	...
	private static List<ScheduleEntry> readSchedule(String fileName, Set<String> onlyRooms, String semester) throws IOException {
		...
		while(true) {
			...
			e.lab = elems[1];
			e.room = elems[5];
			e.courseInstanceID = semester+"/"+e.isvuCode;
			e.kind = "comp";
			e.value = e.lab;
			e.description = "";
			list.add(e);
	...
	static class ScheduleEntry implements Comparable<ScheduleEntry> {
		...
		public String toString() {
			...
			sb.append(isvuCode); sb.append(" ");
			sb.append(courseInstanceID); sb.append(" ");
