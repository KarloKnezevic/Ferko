...
public class AssessmentSummaryViewBuilder extends NavigationBuilder {
	...
	public static void build(Navigation navig, AbstractActionData actionData, boolean root) {
		...
		if(root) {
			navig.getNavigationBar("m2")
				.addItem(
					new TextNavigationItem("Navigation.studentsAssessments")
			);
		} else {
			navig.getNavigationBar("m2")
				.addItem(
					new ActionNavigationItem("Navigation.studentsAssessments", true, "AssessmentSummaryView")
					.addParameter("courseInstanceID", d.getCourseInstance().getId())
			);
		}
	}
