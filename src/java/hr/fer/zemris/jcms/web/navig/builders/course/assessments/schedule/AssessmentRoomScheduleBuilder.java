...
public class AssessmentRoomScheduleBuilder extends NavigationBuilder {
	...
	public static void build(Navigation navig, AbstractActionData actionData, boolean root) {
		...
		if(root) {
			navig.getNavigationBar("m2")
				.addItem(
					new TextNavigationItem("Navigation.assessments.schedule.rooms", true)
				);
		} else {
			navig.getNavigationBar("m2")
				.addItem(
					new ActionNavigationItem("Navigation.assessments.schedule.rooms", true, "AssessmentRoomSchedule")
