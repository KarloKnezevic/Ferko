...
public class ListGroupEventsBuilder extends NavigationBuilder {
	...
	public static void build(Navigation navig, AbstractActionData actionData, boolean root) {
		...
		if(root) {
			navig.getNavigationBar("m2")
				.addItem(
					new TextNavigationItem("Navigation.groupEventsList")
			);
		} else {
			navig.getNavigationBar("m2")
				.addItem(
					new ActionNavigationItem("Navigation.groupEventsList", true, "ListGroupEvents")
