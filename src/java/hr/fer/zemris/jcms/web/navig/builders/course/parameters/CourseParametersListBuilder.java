package hr.fer.zemris.jcms.web.navig.builders.course.parameters;
...
public class CourseParametersListBuilder extends NavigationBuilder {
	...
	public static void build(Navigation navig, AbstractActionData actionData, boolean root) {
		...
		if(root) {
			navig.getNavigationBar("m2")
				.addItem(
					new TextNavigationItem("Navigation.manageCourseParameters")
			);
		} else {
			navig.getNavigationBar("m2")
				.addItem(
					new ActionNavigationItem("Navigation.manageCourseParameters", true, "CourseParametersList")
					.addParameter("courseInstanceID", d.getCourseInstance().getId())
			);
		}
	}
