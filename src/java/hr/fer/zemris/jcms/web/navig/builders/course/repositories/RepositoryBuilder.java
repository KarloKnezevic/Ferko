package hr.fer.zemris.jcms.web.navig.builders.course.repositories;
...
public class RepositoryBuilder extends NavigationBuilder {
	...
	public static void build(Navigation navig, AbstractActionData actionData, boolean root) {
		...
		if(root) {
			navig.getNavigationBar("m2")
				.addItem(
					new TextNavigationItem("Repository.title")
			);
		} else {
			navig.getNavigationBar("m2")
				.addItem(
					new ActionNavigationItem("Repository.title", true, "Repository")
					.addParameter("courseInstanceID", d.getCourseInstance().getId())
			);
		}
	}
